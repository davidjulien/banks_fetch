...
should_db_update_transaction_with_amount(_Config) ->
  ...
  ExpectedTransaction = maps:merge(Transaction11, #{ amount => -25.0, ext_mapping_id => -1, ext_date => {2020,11,16}, ext_period => 'bimester', ext_budget_id => 1, ext_store_id => undefined, ext_categories_ids => [3,4] }),
...
should_db_copy_withdrawal_transaction_to_purse(_Config) ->
  ...
  {ok, [NewTransaction]} = banks_fetch_storage:copy_withdrawal_transaction_to_purse({bank_id, <<"ing">>}, {client_id, <<"client1">>}, {account_id, <<"account1">>}, {transaction_id, <<"transaction2">>}),
  #{account_id := {account_id, <<"purse">>},
    accounting_date := {2020,7,4},
    amount := 20.0,
    bank_id := {bank_id, <<"purse">>},
    client_id := {client_id, <<"David">>},
    description := <<"RETRAIT DAB">>,
    effective_date := {2020,7,4},
    ...
    ext_to_purse := false,
    id := <<"purse-ing-client1-account1-transaction2">>,
    type := card_withdrawal} = NewTransaction,
...
should_db_upgrade_mappings_invalid_updates(_Config) ->
  ...
  Mappings2 = [#{ id => 1, pattern => <<"AUCHAN">>, fix_date => none, period => month, budget_id => 1, categories_ids => [1, 2], store_id => 1 },
               #{ id => 1, pattern => <<"URSSAF">>, fix_date => previous, period => month, budget_id => 1, categories_ids => none, store_id => none } ],
